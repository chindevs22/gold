...
$course_price = SLMS_Course_Price::get( $course_id );
...
$is_trial = (!empty(get_post_meta($course_id, 'shareware', true)));
			...
			if ( empty( $user['id'] ) ) :
                ...
                <?php if($is_trial): ?>
                    <a href="#"
                       ...
                       data-lms-modal="login" style="padding:30px 25px;font-size:18px;">
                        <span class="pull-left"><?php esc_html_e( 'Enroll course', 'masterstudy-lms-learning-management-system-pro' ); ?></span>
                        <span class="pull-right"><?php echo wc_price($course_price); ?></span>
                    </a>
                <?php else: ?>
                ...
                <?php /*if($is_trial): ?>
                    <a href="<?php echo esc_url( $lesson_url ); ?>" class="btn btn-default start-course">
                        <span class="pull-left"><?php echo esc_html( sanitize_text_field( $btn_label ) ); ?></span>
                        <small class="pull-right"><?php echo wc_price($course_price); ?></small>
                    </a>
                <?php else: ?>
                <?php endif;*/ ?>
					...
					<?php if ( ! empty( $price ) || ! empty( $sale_price ) ) : ?>
						<div class="btn-prices btn-prices-price">
							...
							<?php if ( ! empty( $sale_price ) ) : ?>
								<label class="sale_price" title="<?php echo esc_attr( SLMS_Course_Price::display_price( $sale_price ) ); ?>"><?php echo wp_kses_post( SLMS_Course_Price::display_price( $sale_price ) ); ?></label>
							<?php endif; ?>

							<?php if ( ! empty( $price ) ) : ?>
								<label class="price" title="<?php echo esc_attr( SLMS_Course_Price::display_price( $price ) ); ?>"><?php echo wp_kses_post( SLMS_Course_Price::display_price( $price ) ); ?></label>
							<?php endif; ?>
