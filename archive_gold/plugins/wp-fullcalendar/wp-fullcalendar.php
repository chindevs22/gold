...
// 		
		...
		function wpfc_temp_filter_where( $where = '' ) {
			global $wpdb;
			$where .= $wpdb->prepare(' AND wp_postmeta.meta_key = %s AND wp_postmeta.meta_value >= %s', 'event_start_date', $_REQUEST['start']);
			return $where;
		}
...
// 			$post_date = substr($post->post_date, 0, 10);
			if (get_post_meta(get_the_ID(), 'event_start_date', true)) {
				$post_date = get_post_meta(get_the_ID(), 'event_start_date', true);
...
// 			$post_timestamp = strtotime($post->post_date);
	...
	public static function calendar( $args = array() ) {
		...
		error_log("the arguments for the calendar");
		error_log($args);
