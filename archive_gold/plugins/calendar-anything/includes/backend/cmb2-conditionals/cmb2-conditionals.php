...
if (!class_exists('CMB2_Conditionals')) {
    ...
    class CMB2_Conditionals {
        ...
        public function admin_footer() {
            $is_cmcal_setup = $GLOBALS['pagenow'] == "admin.php" && isset($_GET["page"]) && $_GET["page"] == "cmcal_setup";
            $is_cmcal_setup_post_fields = $GLOBALS['pagenow'] == "admin.php" && isset($_GET["page"]) && $_GET["page"] == "cmcal_setup_post_fields";
            $is_cmcal_setup_date_fields = $GLOBALS['pagenow'] == "admin.php" && isset($_GET["page"]) && $_GET["page"] == "cmcal_setup_date_fields";
            if (!in_array($GLOBALS['pagenow'], array('post-new.php', 'post.php'), true) && !$is_cmcal_setup && !$is_cmcal_setup_post_fields && !$is_cmcal_setup_date_fields ) {
                return;
            }

            wp_enqueue_script(
                    'cmb2-conditionals', plugins_url('/cmb2-conditionals.js', __FILE__), array('jquery', 'cmb2-scripts'), self::VERSION, true
            );
        }
        ...
        public function admin_init() {
            ...
            foreach ($cmb2_boxes as $cmb_id => $cmb2_box) {
                add_action(
                        "cmb2_{$cmb2_box->object_type()}_process_fields_{$cmb_id}", array($this, 'filter_data_to_save'), self::PRIORITY, 2
                );
            }
        }
