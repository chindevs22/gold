<?php
    ...
    function create_orders_from_csv($orderData) {
		...
		if (!isset($wp_user_id)) {
			echo "ERROR: No Data for this user";
			error_log("No data for this user: " . $orderData['user_id']);
// 			$wp_user_id = 1; //For testing assign to dadmin user;
			return;
		}
...
// 				error_log("error: Empty P_ID so moving on: "  . $orderData['id']);
// 				echo "error: Empty P_ID so moving on: " . $orderData['id'];
				continue;
			}
            $wp_product_id = get_from_post('product', 'mgml_product_id', $p_id);
			if (empty($wp_product_id) || !isset($wp_product_id)) {
// 				error_log("No product for this ID" . $wp_product_id);
// 				error_log("didn't insert into stm lms order items table " . $orderData['id']);
// 				echo "didn't insert into stm lms order items table for row " . $orderData['id'];
				continue;
			}
...
// 			echo "succesfully inserted into stm lms order items table";
        ...
        foreach($p_ids as $p_id) {
			...
			if (empty($wp_product_id) || !isset($wp_product_id)) {
				error_log("No product for this ID" . $wp_product_id);
				error_log("didn't insert into wp_woocommerce_order_itemmeta table " . $orderData['id']);
				echo "didn't insert into wp_woocommerce_order_itemmeta for row " . $orderData['id'];
				continue;
			}
...
// 			error_log("succesfully inserted into wp_woocommerce_order_itemmeta table");
// 			echo "succesfully inserted into wp_woocommerce_order_itemmeta table";
