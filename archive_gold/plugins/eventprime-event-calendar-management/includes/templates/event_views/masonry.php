...
if($section_id){
    $sec_unique = 'section-masonry-view-'.$section_id;
}
if(isset($atts['class'])){
    $class = $atts['class'];
}
...
<div class="emagic <?php echo $class;?>" id="<?php echo $sec_unique;?>">
    <?php if (!empty($posts)) : ?>
        <div class="em_masonry_view ep-events-masonry-wrap em_cards" id="ms-container">
            <!-- the loop -->
            <?php foreach ($posts as $post) :
                ...
                <div class="em_masonry difl grid-item ep-masonry-item-wrap <?php if (em_is_event_expired($event->id)) echo 'emmasonry-expired'; ?> <?php echo (empty($event->enable_booking) && absint($event->custom_link_enabled) == 0) ? 'em_event_disabled' : ''; ?> <?php echo $column_class;?>" id="em-event-<?php echo $event->id;?>">
                    <div class="em_event_cover_masonry dbfl">
                        <?php 
                    ...
                    <div class="dbfl em-masonry-description-wrap">
                        ...
                        if($event->all_day):?>
                            <div class="ep-masonry-event-date-row">
                                <span class="material-icons em_color">date_range</span> 
                                <div class="ep-masonry-event-date">
                        ...
                        <?php if(!empty($event->description)) { ?>
                                <div class="ep-masonry-event-description dbfl"><?php echo $event->description; ?></div>
                        <?php } ?>
                        <?php 
                        if(!empty($event->venue)){
                            ...
                            if(!empty($venue->id) && !empty($venue->address)){ ?>
                                <div class="ep-masonry-event-address-wrap dbfl" title="<?php echo $venue->address; ?>">
                                    <span class="material-icons em_color">location_on_outline</span><div class="ep-masonry-event-address"><?php echo $venue->address; ?></div></div>
                                <?php 
                            }
                        }
                        ?>
                        <?php if(!empty($event->enable_booking) && empty($event->hide_booking_status)):
                            ...
                            <div class="ep-masonry-booking-row ep-event-attenders-main dbfl">
                    ...
                    <div class="em-masonry-footer dbfl">
        ...
        if(!isset($atts['events'])){
            if($the_query->max_num_pages > 1){
                ...
                <div class="ep-masonry-load-more ep-masonry-load-more-wrap" onclick="em_masonry_load_more_events()" data-curr_page="<?php echo $curr_page?>" data-loading="<?php _e('Loading...');?>" data-loaded="<?php _e('Load More');?>" data-max_page="<?php echo $the_query->max_num_pages;?>" data-upcoming="<?php echo $upcoming;?>" data-sites="<?php echo $sites;?>" data-types="<?php echo $types;?>" data-show="<?php echo $posts_per_page;?>" data-recurring="<?php echo $recurring;?>" data-i_events="<?php echo $i_events;?>"><div class="ep-load-more-button em_color"><?php _e('Load More');?></div></div><?php
            }
        }?>
    <?php else: ?>
        <?php if($_POST): ?>
            ...
            if ( container ) {
                ...
                imagesLoaded( container, function() {
                    msnry = new Masonry( container, {
                        itemSelector: '#ms-container<?php echo $section_id;?> .grid-item',
                    });
                });
            }
        });
        
        em_width_adjust("#ms-container<?php echo $section_id;?> .ep-masonry-item-wrap");
        jQuery(this).prop("disabled", true);
    });
</script>
<style type="text/css">
    .emagic .em_masonry.desktop-col-4 {
        width: 23%;
    }
</style>