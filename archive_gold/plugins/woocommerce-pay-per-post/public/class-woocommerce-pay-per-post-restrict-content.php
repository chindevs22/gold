...
class Woocommerce_Pay_Per_Post_Restrict_Content
{
    ...
    */
            ...
            foreach ( $this->product_ids['product_ids'] as $id ) {
                ...
                if ( Woocommerce_Pay_Per_Post_Helper::can_use_woocommerce_subscriptions() ) {
                    ...
                    if ( wc_customer_bought_product( $this->current_user->user_email, $this->current_user->ID, trim( $id ) ) && !$this->integrations['woocommerce-subscriptions']->is_subscription_product( $id ) ) {
                        Woocommerce_Pay_Per_Post_Helper::logger( 'Post ID: ' . get_the_ID() . ' - Woocommerce_Pay_Per_Post_Restrict_Content/check_if_purchased  - WooSubscriptions Enabled and User has purchased product id #' . trim( $id ) . ' that is NOT a subscription product' );
                        return true;
                    }
    ...
    protected function shortcode_expiration_status( $template )
    {
        ...
        if ( isset( $user_info['last_purchase_date'] ) && !is_null( $user_info['last_purchase_date'] ) ) {
            ob_start();
