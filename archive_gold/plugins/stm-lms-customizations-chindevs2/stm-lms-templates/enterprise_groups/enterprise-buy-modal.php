...
$price = SLMS_Course_Price::get_enterprise( $course_id );
$currency = SLMS_Course_Price::get_currency( $course_id );
...
<div class="actions <?php echo ( empty( $groups ) ) ? 'no-groups' : 'have-groups'; ?>">
	<a href="#"
       ...
       data-enterprise-price="<?php echo esc_attr( $price ); ?>"
		data-enterprise-currency="<?php echo esc_attr( $currency ); ?>">
		<?php
		printf(
			/* translators: %s Price */
			esc_html__( 'Add to cart %s', 'masterstudy-lms-learning-management-system-pro' ),
			'<span>' . SLMS_Course_Price::display_price( '0', $course_id ) . '</span>' // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		);
		?>
	</a>
