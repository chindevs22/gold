...
! defined( 'WOONP_VERSION' ) && define( 'WOONP_VERSION', '2.0.4' );
		...
		if ( ! class_exists( 'WPCleverWoonp' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWoonp {
				...
				function __construct() {
					...
					add_action( 'before_woocommerce_init', function () {
						if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) ) {
							FeaturesUtil::declare_compatibility( 'custom_order_tables', WOONP_FILE );
						}
					} );
				}
							...
							<?php if ( $active_tab === 'settings' ) {
								...
								<form method="post" action="options.php">
									<table class="form-table">
										...
										<tr>
											...
											<td>
												<select name="woonp_settings[global_status]">
													<option
														value="enable" <?php selected( $global_status, 'enable' ); ?>><?php esc_html_e( 'Enable', 'wpc-name-your-price' ); ?></option>
													<option
														value="disable" <?php selected( $global_status, 'disable' ); ?>><?php esc_html_e( 'Disable', 'wpc-name-your-price' ); ?></option>
												</select>
												<span
													class="description"><?php esc_html_e( 'You still can enable/disable it on a product basis.', 'wpc-name-your-price' ); ?></span>
											</td>
										</tr>
										<tr>
											...
											<td>
												<input type="text" name="woonp_settings[suggested_price]"
												       value="<?php echo WoonpHelper::get_setting( 'suggested_price', esc_html__( 'Suggested Price: %s', 'wpc-name-your-price' ) ); ?>"/>
												<span
													class="description"><?php esc_html_e( 'Use General tab\'s price as suggested price, leave blank to hide. Use "%s" for price.', 'wpc-name-your-price' ); ?></span>
											</td>
										</tr>
										<tr>
											...
											<td>
												<select name="woonp_settings[atc_button]">
													<option
														value="show" <?php selected( $atc_button, 'show' ); ?>><?php esc_html_e( 'Show', 'wpc-name-your-price' ); ?></option>
													<option
														value="hide" <?php selected( $atc_button, 'hide' ); ?>><?php esc_html_e( 'Hide', 'wpc-name-your-price' ); ?></option>
												</select>
												<span
													class="description"><?php esc_html_e( 'Show/hide add to cart button on the shop/archive page.', 'wpc-name-your-price' ); ?></span>
											</td>
										</tr>
										<tr>
											...
											<td>
												<select name="woonp_settings[rounding]">
													<option
														value="down" <?php selected( $rounding, 'down' ); ?>><?php esc_html_e( 'Down', 'wpc-name-your-price' ); ?></option>
													<option
														value="up" <?php selected( $rounding, 'up' ); ?>><?php esc_html_e( 'Up', 'wpc-name-your-price' ); ?></option>
												</select>
												<span
													class="description"><?php esc_html_e( 'Round the amount to the nearest bigger (up) or smaller (down) value when an invalid number is inputted.', 'wpc-name-your-price' ); ?></span>
											</td>
										</tr>
										<tr class="heading">
											<th>
										...
										<tr>
											...
											<td>
												<input type="text" name="woonp_settings[label]"
												       value="<?php echo WoonpHelper::get_setting( 'label', esc_html__( 'Name Your Price (%s) ', 'wpc-name-your-price' ) ); ?>"/>
												<span
													class="description"><?php esc_html_e( 'Use "%s" for currency.', 'wpc-name-your-price' ); ?></span>
											</td>
										</tr>
										<tr>
											...
											<td>
												<select name="woonp_settings[value]">
													<option
														value="price" <?php selected( $value, 'price' ); ?>><?php esc_html_e( 'Product price', 'wpc-name-your-price' ); ?></option>
													<option
														value="min" <?php selected( $value, 'min' ); ?>><?php esc_html_e( 'Min value', 'wpc-name-your-price' ); ?></option>
													<option
														value="max" <?php selected( $value, 'max' ); ?>><?php esc_html_e( 'Max value', 'wpc-name-your-price' ); ?></option>
													<option
														value="empty" <?php selected( $value, 'empty' ); ?>><?php esc_html_e( 'Empty', 'wpc-name-your-price' ); ?></option>
												</select>
											</td>
										</tr>
										<tr>
											...
											<td>
												<select name="woonp_settings[type]" class="woonp_type">
													<option
														value="default" <?php selected( $type, 'default' ); ?>><?php esc_html_e( 'Input (default)', 'wpc-name-your-price' ); ?></option>
													<option
														value="select" <?php selected( $type, 'select' ); ?>><?php esc_html_e( 'Select', 'wpc-name-your-price' ); ?></option>
												</select>
											</td>
										</tr>
										<tr class="woonp_show_if_type_input">
											...
											<td>
												<input type="number" name="woonp_settings[min]" min="0"
												       ...
												       value="<?php echo WoonpHelper::get_setting( 'min' ); ?>"/>
												<span
													class="description"><?php esc_html_e( 'Leave blank or zero to disable.', 'wpc-name-your-price' ); ?></span>
											</td>
										</tr>
										<tr class="woonp_show_if_type_input">
											...
											<td>
												<input type="number" name="woonp_settings[step]" min="0"
												       step="<?php echo esc_attr( $step ); ?>"
												       value="<?php echo WoonpHelper::get_setting( 'step' ); ?>"/>
												<span
													class="description"><?php esc_html_e( 'Leave blank or zero to disable.', 'wpc-name-your-price' ); ?></span>
											</td>
										</tr>
										<tr class="woonp_show_if_type_input">
											...
											<td>
												<input type="number" name="woonp_settings[max]" min="0"
												       step="<?php echo esc_attr( $step ); ?>"
												       value="<?php echo WoonpHelper::get_setting( 'max' ); ?>"/>
												<span
													class="description"><?php esc_html_e( 'Leave blank or zero to disable.', 'wpc-name-your-price' ); ?></span>
											</td>
										</tr>
										<tr class="woonp_show_if_type_select">
											...
											<td>
                                                <textarea name="woonp_settings[values]" rows="10"
                                                          cols="50"><?php echo WoonpHelper::get_setting( 'values' ); ?></textarea>
												<p class="description">
										...
										<tr class="submit">
											<th colspan="2">
												<?php settings_fields( 'woonp_settings' ); ?>
												<?php submit_button(); ?>
											</th>
										</tr>
									</table>
				...
				function product_data_panels() {
					...
					$post_id = $post->ID;
					$step    = '0.0001';
					$price   = get_post_meta( $post_id, '_woonp_status', true ) ?: 'default';
					$type    = get_post_meta( $post_id, '_woonp_type', true ) ?: 'default';
					?>
						...
						<div class="woonp_tr">
							...
							<div class="woonp_td">
								<input name="_woonp_status" type="radio"
								       value="default" <?php echo esc_attr( $price === 'default' ? 'checked' : '' ); ?>/> <?php esc_html_e( 'Default',
									'wpc-name-your-price' ); ?>
								(<a
								...
								&nbsp;
								<input name="_woonp_status" type="radio"
								       value="disable" <?php echo esc_attr( $price === 'disable' ? 'checked' : '' ); ?>/> <?php esc_html_e( 'Disable',
									'wpc-name-your-price' ); ?>
								&nbsp;
								<input name="_woonp_status" type="radio"
								       value="overwrite" <?php echo esc_attr( $price === 'overwrite' ? 'checked' : '' ); ?>/> <?php esc_html_e( 'Overwrite',
									'wpc-name-your-price' ); ?>
							</div>
						</div>
							...
							<div class="woonp_tr">
								...
								<div class="woonp_td">
									<select name="_woonp_type">
										<option
											value="default" <?php echo esc_attr( $type === 'default' ? 'selected' : '' ); ?>><?php esc_html_e( 'Input (default)', 'wpc-name-your-price' ); ?></option>
										<option
											value="select" <?php echo esc_attr( $type === 'select' ? 'selected' : '' ); ?>><?php esc_html_e( 'Select', 'wpc-name-your-price' ); ?></option>
									</select>
								</div>
							</div>
